...
from gbdt_train_plus import load_data
...
def xgb_param_adjust(train_data, label):
    ...
    # }
    param_test3 = {
        'subsample': [i / 100.0 for i in range(75, 90, 1)],
        'colsample_bytree': [i / 100.0 for i in range(75, 90, 1)]
    }
    gsearch1 = GridSearchCV(estimator=XGBClassifier(learning_rate=0.01, n_estimators=14, max_depth=4,
                                                    min_child_weight=1, gamma=0, reg_lambda=1, subsample=0.75, colsample_bytree=0.8,
